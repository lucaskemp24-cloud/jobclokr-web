...
export default function ReportsPage() {
  ...
  const filteredEntries = useMemo(() => {
    ...
    return entries.filter((entry) => {
      ...
      const matchesDate =
        clockInDate >= start && clockInDate <= end;
  ...
  }, [entries, startDate, endDate, selectedEmployeeId]);
  ...
  const employeeSummary = useMemo(() => {
    ...
    filteredEntries.forEach((entry) => {
      const current = totals.get(entry.employeeName) ?? {
      ...
      totals.set(entry.employeeName, current);
    });
  ...
  const projectSummary = useMemo(() => {
    ...
    filteredEntries.forEach((entry) => {
      const current = totals.get(entry.projectName) ?? {
      ...
      totals.set(entry.projectName, current);
    });
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="mt-1 text-gray-500">
              Review employee and project labor hours.
            </p>
          </div>
